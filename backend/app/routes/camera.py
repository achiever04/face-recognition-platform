...
def process_camera_sync(cam_id: int, cap: cv2.VideoCapture, config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Optionally enforce camera fps limit: skip processing if system is too fast
        fps_limit = CAMERA_METADATA.get(cam_id, {}).get("fps_limit")
        if fps_limit:
            elapsed = time.time() - start_time
            # minimal enforcement: if we've done processing too quick, sleep a small amount
        ...
        for face_encoding in faces_enc:
            try:
                ...
                for match in matches:
                    # match["match"] is boolean; match["distance"] numeric; match["confidence"] may be provided
